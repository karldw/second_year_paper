...
\widowpenalty=9999  % as bad as possible without being 'infinitely' bad
...
Let $W_0$ be the start of the anticipation window and $W_1$ be the end, so $[W_0, W_1]$ is the window and $\anticipation$ is a dummy for $t$ in the window.
Though I'm calling it an ``anticipation window,'' I'm going to estimate some versions with $W_0$ or $W_1 > 0$.
For brevity, let $F{\, sty}$ be the vector of state and year fixed effects, 17 in all.
...
The terms to be estimated are the $\alpha_i$, $A$, $\beta_i$, $B$, $\gamma_i$, $\Gamma$, $\delta_i$, $\Delta$, $\zeta_i$, $Z$, $\eta_i$, $H$, $\theta_i$, $\Theta$, $\lambda_i$, $\Lambda$ (for $i = 1,2,3,4$).
\begin{align}
    \label{eqn:cars-sold}
...
% TODO talk about sale price here too.
...
\begin{figure}[!hbtp]
    ...
    % TODO: put in the appropriate greek letter.
    }
\end{figure}
